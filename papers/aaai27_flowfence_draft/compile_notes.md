...
- Result: both commands returned `command not found`; compilation still needs to be checked in an environment with a LaTeX distribution.
- PDF produced: no.
...
- v1 reference count: 10.
- Placeholder `Anonymous` author entries remaining: 0.
- Remaining TODO citations in `sections/07_related_work.tex`: 0.
- Bibliography metadata was checked against arXiv/ACL Anthology pages where available, but final venue formatting still needs human verification before submission.
