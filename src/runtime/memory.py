"""Simple in-memory artifact store for synthetic MAS traces."""
...
MEMORY_ZONES = {"private", "shared_raw", "safe_view", "quarantine"}
...
class MemoryStore:
    ...
    def write(
        ...
        *,
        ...
        if zone not in MEMORY_ZONES:
            raise ValueError(f"Unsupported memory zone: {zone}")
        self._counter += 1
        artifact_id = f"mem_{self._counter:04d}"
