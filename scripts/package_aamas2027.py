...
def generate_reports(root: Path) -> dict[str, Any]:
    ...
    if e1_tied:
        b_conclusion = (f"PAPC and IFC-SafeView tie in all {e1_overall_pairs['success']['matched']} matched groups on task success, privacy-safe success, raw/external exposure, legacy policy-violation observer pairs, interventions and blocks. The live three-agent slice provides no measured PAPC advantage over equal-capability IFC-SafeView.")
    ...
    paper = ["# AAMAS paper integration", "", "Use only the completed rows below. All rates are proportions; exposure/intervention columns are episode means unless stated otherwise.", "", "## Table A — Equal-capability deterministic benchmark", "",
             markdown_table(data["e0_table"], [("defense", "Defense"), ("episodes", "n"), ("success", "Task"), ("privacy_safe_success", "Safe task"), ("raw_exposure", "Raw"), ("external_exposure", "External"), ("exposure_recipient_pairs", "LEGACY observer pairs"), ("intervention_count", "Interventions"), ("blocks", "Blocks")]), "",
