#!/usr/bin/env python3
"""Read saved D3 evidence only; recompute descriptive audits without model/evaluator calls."""
import argparse
from collections import Counter,defaultdict
import json
from pathlib import Path
from src.e2_live.d3 import D3,admit_d3
...
def main():
    ...
    episodes=rows(a.run/'episodes.jsonl');attempts=rows(a.run/'attempts.jsonl');schedule=load(D3/'CELL_SCHEDULE.json')
    ...
    summary={'label':LABEL,'attempted':len(attempts),'finished':len(episodes),'valid':sum(e['valid'] for e in episodes),'completed_finals':sum(e.get('completed_final',False) for e in episodes),
        ...
        'paired_initial_live_requests_verified':pair_count,'run_complete':completed,'readiness':'READY_FOR_CONFIRMATORY_REVIEW' if completed and len(episodes)==54 and not defects else 'NOT_READY',
    ...
    summary.update(treatment_entry_given_valid_handoff={'entered':entries,'valid_finance_handoff':valid_handoffs,'rate':entries/valid_handoffs if valid_handoffs else None},unconditional_valid_handoff={'valid_finance_handoff':valid_handoffs,'scheduled':36,'rate':valid_handoffs/36})
