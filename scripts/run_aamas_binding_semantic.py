#!/usr/bin/env python3
"""Run preregistered E6 with exclusive outputs, a shared API budget and no retries."""
...
from src.experiments.aamas_binding_semantic import BindingEpisode,BindingDryClient,load_config,matrix_cells
...
class RequestBudget:
    def __init__(self,limit):
        self.limit=limit; self.attempts=0; self.stop_reason=None; self.infrastructure_failures=0; self.lock=threading.Lock()

    def request(self,client,system,prompt):
        with self.lock:
            if self.stop_reason: raise ProviderFailure('NOT_ATTEMPTED_'+self.stop_reason)
            if self.attempts>=self.limit:
                self.stop_reason='BUDGET_EXHAUSTED'; raise ProviderFailure('NOT_ATTEMPTED_BUDGET_EXHAUSTED')
            self.attempts+=1
        try:
            response=client.complete_messages(system,prompt)
            with self.lock: self.infrastructure_failures=0
            return response
        except ProviderFailure as e:
            with self.lock:
                if e.kind in ('HTTP_401','HTTP_403','BLOCKED_BY_API'): self.stop_reason=e.kind
                elif e.kind.startswith(('HTTP_5','HTTP_429','PROVIDER_')):
                    self.infrastructure_failures+=1
                    if self.infrastructure_failures>=3: self.stop_reason='CONSECUTIVE_INFRASTRUCTURE_FAILURES'
            raise


class BudgetClient:
    def __init__(self,client,budget): self.client=client; self.budget=budget
    def available(self): return self.client.available()
    def complete_messages(self,system,prompt): return self.budget.request(self.client,system,prompt)
...
def run(config,tasks,profile,hashes,output,private_output=None,dry_run=False,source_sha=None,provider_settings=None):
    ...
    lock=threading.Lock(); rows=[]; budget=RequestBudget(config['max_provider_requests'])
    def sink(kind,row):
    ...
    def job(cell):
        ...
        client=BindingDryClient() if dry_run else BudgetClient(MiniMaxAgentClient(model=config['model'],temperature=gen['temperature'],max_tokens=gen['max_tokens'],timeout_seconds=gen['timeout_seconds'],provider_settings=settings),budget)
