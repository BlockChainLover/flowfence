...
CONFIG_PATH="${1:-configs/experiment/asb_smoke.yaml}"
RUN_NAME="${2:-}"
...
ssh "${REMOTE_HOST}" "bash -lc \"cd '${REMOTE_REPO}' && PYTHONPATH='${REMOTE_REPO}' '${REMOTE_VENV}/bin/python' src/runner/run_asb_smoke.py --config '${CONFIG_PATH}'${RUN_ARG}\""
