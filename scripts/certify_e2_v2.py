#!/usr/bin/env python3
"""Deterministic V2 mediation/parity certification; synthetic runtime and D2 ingress only."""
import argparse
...
from scripts.check_e2_v2_runtime import fixture, FINALS, DELEGATE, HANDOFF
...
from src.e2_live.v2 import EpisodeV2, build_request
...
def primed(root,family,arm,name,policies=None):
    ...
    ep=EpisodeV2(cell,task,policy,{},FakeProvider([DELEGATE,HANDOFF,{'action':'history'}]),FakeEvaluator(),root/name)
    # Replace only trusted fixture broker; actual same Runtime B4/B5 path remains.
    ...
    for _ in range(3):
        actor=ep.scheduler.next();snap=ep.snapshot(actor);action=ep.provider_action(actor,snap)
        value=ep.service(actor,action['action'],**{k:v for k,v in action.items() if k!='action'})
        ep.scheduler.after(actor,action)
    assert ep.scaffold.complete and ep.scaffold.invoked==set(PRINCIPALS)
    return ep
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for family in FINALS:
            ...
            for arm in ('EXACT_IFC','FLOWFENCE_R2'):
                ...
                c.finalize(FINALS[family]);native=rt.evaluator_output();c.stop();denied(c.context)
                ...
                arms.append((rt.configuration(),rt.audit(True),rt.transitions(),native,ep.scaffold.events))
        ...
        # D2-only public input serialization, no source evaluator or generated answers.
        d2=Path('artifacts/aamas2027_e2_v2_d2');rows=load(d2/'development.json');schedule=load(d2/'CELL_SCHEDULE.json')
        ...
        ingress=[];by_pair={};d2_raw=[]
        for cell in schedule:
            key=(cell['family'],cell['task_id']);ep=EpisodeV2(cell,tasks[key],policies[key],load(a.schema),FakeProvider([]),FakeEvaluator(),root/cell['cell_id'])
            d2_raw.append(ep.pol.raw_value)
            snap=ep.snapshot('planner_agent');req=build_request('planner_agent',cell['family'],snap)
        ...
        base='3fb3e2d7ef36509e96adba6784faa32f703f7896'
        protected=['src/e2_live/pilot.py','scripts/run_e2_development_pilot.py','E2_DEVELOPMENT_PILOT_REPORT.md']
        protected+=subprocess.check_output(['git','ls-tree','-r','--name-only',base,'artifacts/aamas2027_e2_development_live','artifacts/aamas2027_e2_source_s1f']).decode().splitlines()
        ...
        checks.append('54 D2 public initial requests serialize; 27 arm pairs identical; no private facts in planner context; no provider call')
        out={'status':'VERIFIED','V2_STRUCTURAL_PARITY':'VERIFIED','V2_MEDIATION_INTEGRITY':'VERIFIED',
            'transition_classes_verified':sorted(classes),'transition_classes_total':23,'boundary_coverage':sorted(boundaries),
            'modeled_bypasses_remaining':0,'paired_scenarios':pairs,'checks':checks,'D2_initial_ingress':ingress,
            'preserved_V1_files':len(protected),'frozen_integrity':pins,'V1_results_recomputed':False,
            ...
            'D2_MODEL_RUNS_EXECUTED':0,'CONFIRMATORY_TASKS_EXECUTED':0,'FORMAL_MODEL_RUNS_EXECUTED':0}
        encoded=json.dumps(out,indent=2)+'\n'
        assert not any(raw in encoded for raw in d2_raw)
    a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(encoded)
    print(json.dumps({'status':'VERIFIED','transitions':len(classes),'boundaries':len(boundaries),'pairs':len(pairs),'D2_initial_requests':54,'live_calls':0}))
