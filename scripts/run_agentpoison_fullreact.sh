...
ssh "${REMOTE_HOST}" "bash -lc \"cd '${REMOTE_REPO}' && PYTHONPATH='${REMOTE_REPO}' '${REMOTE_VENV}/bin/python' src/runner/run_agentpoison_fullreact.py --config '${CONFIG_PATH}'${RUN_ARG}\""
rsync -avz "${REMOTE_HOST}:${REMOTE_REPO}/results/" "./results/"

echo "agentpoison_fullreact_complete"
