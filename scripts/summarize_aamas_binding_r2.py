#!/usr/bin/env python3
"""Rebuild PAPC-R2 operational and complete-pair scientific reports from safe records."""
...
def summarize(folder,output):
    ...
    for r in rows:
        ...
        rebuilt['safe_view_rewrites']=sum(e['decision']=='rewrite_safe_view' and e['content_changed'] for e in es)
        rebuilt['quarantines']=sum(e['decision']=='quarantine' for e in es)
        ...
        r.update(rebuilt)
    groups=[]
    for topology in ('overall','blackboard_4'):
    ...
    matched=[(r,indexed[(r['task_id'],r['topology'],r['condition'],'ifc_safeview',r['seed'])]) for r in rows if r['defense']=='flowfence_lite_nonoracle_r2' and (r['task_id'],r['topology'],r['condition'],'ifc_safeview',r['seed']) in indexed]
    for topology in ('overall','blackboard_4'):
        for condition in ('overall',*registration['config']['conditions']):
    ...
    result['formal_evidence_complete']=not registration['dry_run'] and all(n==6 for n in coverage.values()) and len(rows)==36 and all(r["status"]=="completed" for r in rows) and all(n>0 for ds in hold_coverage.values() for n in ds.values())
    ...
    lines=['# PAPC-R2 discrimination results','',result['scope'],'',f"Cells: {len(rows)}/{len(expected)}. Status: {result['status_counts']}. API attempts: {len(actual)}. Tokens: {result['input_tokens']} input / {result['output_tokens']} output. Retries: {result['retries']}.",'','Values below are counts summed across retained first attempts, not per-episode means.','', '| Defense | n | Task | Safe task | Delivered exact | Delivered reconstructable | Exact attempts | Reconstructable attempts | Unauthorized pairs | Forbidden secret-events | Interventions | Blocks | Safe-view rewrites | Quarantines | Parsed exact attempts | Parsed reconstructable attempts |','|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|---|']
    for g in groups:
