#!/usr/bin/env python3
"""Audit R2 pilot paths, quarantine release and private-input boundaries; emit booleans only."""
import argparse
import json
from pathlib import Path
from src.experiments.aamas_binding_r2 import check_pilot, load_config
...
def audit(folder, private):
    config, _, _, _ = load_config(Path('configs/experiment/aamas2027/r2_pilot.json'))
    try:
    ...
    tail=profile['attacks']['registered_semantic_request'].split('. ',1)[1]
    p2=[r for r in rows if 'e6_b05' in r['run_id']]
    source_events=[r for r in p2 if r.get('stage')=='mediated_event' and r['event']['origin']=='exogenous_source']
    marker_only=bool(source_events) and all(r['delivered_content']==QUARANTINE_MARKER for r in source_events)
    residual_absent=all(tail not in r['prompt'] for r in p2 if r.get('stage')=='request')
    gate=gate and marker_only and residual_absent
    ...
    result = {'pilot_gate_pass': gate and boundary and permissions, 'paths_and_responses_pass': gate,
              ...
              'p2_source_release_marker_only':marker_only,'p2_residual_instruction_absent':residual_absent,
