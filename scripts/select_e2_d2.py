#!/usr/bin/env python3
"""Precommitted metadata-only D2 selection; no model/source-content/evaluator access."""
import argparse
...
RULE_FILES=('E2_D2_SELECTION_RULE.md','scripts/select_e2_d2.py')
FROZEN=Path('artifacts/aamas2027_e2_source_s1f')
OUT=Path('artifacts/aamas2027_e2_v2_d2')
...
def select(meta, exclusions):
    ...
    assert min(len(bird),len(tat),len(available))>=3,'INSUFFICIENT_D2_DIVERSITY_NO_RELAXATION'
...
def main():
    ...
    assert not a.output.exists(),'NO_OVERWRITE_SELECTED_D2'
    ...
    old=load(FROZEN/'development.json')+load(FROZEN/'confirmatory.json')
    exclusions={(r['family'],r['task_id']) for r in old};assert len(exclusions)==69
    ...
    for family in ('bird_pg','tatqa','hotpot'):
        for position,(tid,typ) in enumerate(zip(selected[family],config['dev_types'][family]),1):
            ...
            rows.append({'namespace':'E2_DEVELOPMENT_V2_D2','split':'development_d2','permanently_development_only':True,
    ...
    for ordinal,row in enumerate(sorted(rows,key=lambda r:(r['family'],r['task_id'])),70):
        policy={k:deepcopy(v) for k,v in templates.items() if k not in ('fact_templates','ordinal_rule','secret_id_rule','limitations')}
        policy.update(family=row['family'],task_id=row['task_id'],split='development_d2',namespace='E2_DEVELOPMENT_V2_D2',fact_type=row['fact_type'],
    ...
    for c in range(3):
        for t,row in enumerate(sorted(rows,key=lambda r:(r['family'],r['task_id']))):
            ...
            for arm in arms:
                n=len(schedule)+1
                schedule.append({'namespace':'E2_DEVELOPMENT_V2_D2','cell_id':f'E2_DEVELOPMENT_V2_D2_{n:03d}','order':n,
    ...
    save(a.output/'selection_provenance.json',{'D2_SELECTION_RULE_COMMIT':commit,'namespace':'E2_DEVELOPMENT_V2_D2',
        'eligible_pool':config['eligibility'],'cluster_metadata':config['clusters'],'excluded_ids':69,'D2_TASKS_SELECTED':9,
        'D2_DISJOINT_FROM_V1_DEV':True,'D2_DISJOINT_FROM_CONFIRMATORY':True,'ranking_inputs':'IDs,compatible types,cluster metadata,exclusion lists only',
        'source_content_read':False,'model_outputs_read':False,'raw_values_instantiated':False,
        'confirmatory_quotas_unchanged':{'P1':15,'P2':15,'P3':10,'P4':20},'D2_MODEL_RUNS_EXECUTED':0})
